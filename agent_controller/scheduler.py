...
Behavior:
- Does not split tasks. Tasks with duration > block_max are flagged with
  split_recommended=True
- Uses greedy bin-packing to balance day workloads deterministically.
- Returns a list of schedule entries with day index and start/end times
  (ISO strings if start_date provided).
...
def _assign_tasks_to_days(converted, capacity_per_day, block_max):
    """Helper to assign tasks to days using greedy bin-packing."""
...
def create_schedule(
    ...
    Parameters:
    - tasks: iterable of task dicts (must contain 'id' and estimate_hours or
      estimate_minutes)
    ...
    Returns list of entries:
      {
        ...
        'start_ts': optional ISO str,
        'end_ts': optional ISO str
      }
    """
    ...
    if capacity_per_day <= 0:
        raise ValueError(
            "availability results in non-positive capacity; adjust hours_per_day "
            "or buffer_ratio"
        )
    ...
    days = _assign_tasks_to_days(converted, capacity_per_day, block_max)
