...
- apply_create_events(credentials, events, dry_run=True) -> create events, return created event ids

Uses google-api-python-client when available; otherwise functions still work in dry-run mode
and raise a RuntimeError if apply_create_events is called with dry_run=False and no libraries.
...
def make_event_objects(schedule: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    Schedule rows expected to have: task_id, start_ts (ISO), end_ts (ISO), title, description.
    ...
    for s in schedule:
        ev = {
            ...
            "start": {"dateTime": s.get("start_ts"), "timeZone": s.get("time_zone", "UTC")},
...
def _request_with_retry(func, *args, max_retries: int = 5, backoff_base: float = 0.5, **kwargs):
    attempt = 0
...
def apply_create_events(credentials: Any, events: Iterable[Dict[str, Any]], calendar_id: str = "primary", dry_run: bool = True) -> List[Dict[str, Any]]:
    """Create events on Google Calendar.

    - credentials: google oauth2 credentials object (from oauth_helper.load_credentials/run_local_oauth_flow)
    ...
    - Download client_secret.json and use `helpers.oauth_helper.run_local_oauth_flow` to obtain credentials.
    """
    ev = list(events)
    if dry_run:
    ...
    if not _HAS_GOOGLEAPI or credentials is None:
        raise RuntimeError("google-api-python-client is required to create events. Install google-api-python-client and oauth libraries.")
